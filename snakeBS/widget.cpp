    ...
    ui(new Ui::Widget)
{
	...
	qDebug() << "QApplication::applicationDirPath()===" << QApplication::applicationDirPath();//exe文件所在的绝对路径
	QString musicPath = QString("%1/music.wav").arg(QApplication::applicationDirPath());
    sound=new QSound(musicPath);
    sound->play();
    sound->setLoops(-1);//播放音乐

    ui->label->setText(QString("0"));
    this->setWindowTitle("Snake");
    this->setWindowIcon(QIcon(":/new/prefix1/image/love1.ico"));//执行文件的图标
    qsrand(QTime(0,0,0).secsTo(QTime::currentTime()));
    direction=1;
    speed=500;
    grades=0;
    arr[0][0]=1+qrand()%(49-1);//利用伪随机函数来随机生成一开始蛇头的位置和食物的位置
    arr[0][1]=1+qrand()%(49-1);
    food_x=1+qrand()%(49-1);
    food_y=1+qrand()%(49-1);
    m_timer=new QTimer(this);
    connect(m_timer,&QTimer::timeout,this,&Widget::timeslot);//用于信号和槽的连接
...
void Widget::paintEvent(QPaintEvent *)//绘制背景，蛇，食物
{
    QPainter p(this);
    ...
    //画蛇身
    int j;
    for(j=0;j<=grades;j++)//颜色，红色，每节不同的颜色，整体不停地改变颜色，每节改变不同的颜色，带有光环特效的
    {
        if(str1=="red")
        {
            brush.setColor(Qt::red);
        ...
        else if(str1=="colorful")
        {
            ...
            else if(j%5==1)
            {
                ...
                p.setPen(pen);
                brush.setColor(Qt::yellow);
                brush.setStyle(Qt::SolidPattern);//定义了填充模式
                p.setBrush(brush);
            }
            else if(j%5==2)
            {
...
void Widget::keyPressEvent(QKeyEvent *e)
{
    ...
    else if(non_human_record==1)//非人类模式
    {
        switch (e->key()) {
        case Qt::Key_W:
            if(grades==0)
...
void Widget::timeslot()//用二维数组记录蛇头的位置和蛇身的长度
{
    changeable_record++;
    int j;
    for(j=1;j<=grades;j++)
    {
        if(arr[0][0]==arr[j][0]&&arr[0][1]==arr[j][1])//头的坐标和尾的坐标重合，游戏结束
        {
            m_timer->stop();
            this->hide();
            form.show();
    ...
    if(arr[0][0]==food_x&&arr[0][1]==food_y)//吃到食物
    {
        grades+=1;
    ...
    if(arr[0][0]<1||arr[0][0]>=49||arr[0][1]<1||arr[0][1]>=49)//蛇头出去或碰到墙壁游戏结束
    {
        m_timer->stop();
        this->hide();
        form.show();
...
void Widget::play_again_deal()//再玩一次
{
    this->show();
    form.hide();
    init_all();
...
void Widget::init_all()//最开始的蛇和食物的随机位置
{
    direction=1;
    grades=0;
...
void Widget::time_open(int level_speed)//难度
{
    speed=level_speed;
}

void Widget::food_deal(QString str)//食物
{
    str2=str;
}

void Widget::on_pushButton_clicked()//3个按键的事件链接
{
    m_timer->start(speed);
